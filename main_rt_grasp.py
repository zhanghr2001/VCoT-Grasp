...
def main(args):
    ...
    if not args.load_checkpoint_dir:
        ...
        config = VCoTGraspConfig.from_json_file("./model/config.json")
        config.arch_config = ArchConfig(use_bbox=False, action_head="LM_new")
        model = VCoTGraspForConditionalGeneration(config)
        model = PeftModel.from_pretrained(model, args.load_checkpoint_dir, is_trainable=True)
        model.print_trainable_parameters()
        processor = VCoTGraspProcessor(arch_config)
    ...
    if args.resume:
        ...
        accelerator.load_state(args.resume_checkpoint_dir)
